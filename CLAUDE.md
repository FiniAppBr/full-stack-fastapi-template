...
- **Package Managers**: Python venv/uv, npm (Node.js)
...
- Frontend: `/opt/connectai/frontend/.env.local` (if exists)
...
4. No need to systemctl restart after every change during development
5. ⚠️ **Change default passwords** in `.env` before production use
